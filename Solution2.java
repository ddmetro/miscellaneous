...
public class Solution2 implements Runnable 
{
	...
	int[] l;//ampere cost
	int[] g;//installation cost for generator
	int[] generatorVertex;
	...
	public void run()
	{
		try
		{
			...
			for(int t = 0; t < T; t++)
			{
				...
				for(int count = 0; count < N; count++)
				{
					l[count+1] = parser.nextInt();
				}

				for(int count = 0; count < N; count++)
				{
					g[count+1] = parser.nextInt();
				}
	...
	{	
		...
		for(int count = 0; count < cmp; count++)
		{
			...
			for(int vCount = 0; vCount < component[count+1].size(); vCount++)
			{
				...
				p.adj[node] = adj[node];
				if(minCost > (l[node] + g[node]))
				{
					minCost = l[node] + g[node];
					minVertex = node;
				}
			}
			...
			generatorVertex[count+1] = minVertex;
		}
		...
		for(int count = 0; count < cmp; count++)
		{
			output.print(generatorVertex[count+1] + " ");
		}
		output.print("\n");

		for(int count = 0; count < N; count++)
		{
			output.print(generatorVertex[mark[count+1]] + " ");
		}
